        ...
        for raw in notes or []:
            ...
            coerced.append(
                SourceNote(
                    ...
                    reason_codes=list(getattr(raw, "reason_codes", []) or []) if not isinstance(raw, dict) else list(raw.get("reason_codes") or []),
                    candidate_id=(raw.get("candidate_id") if isinstance(raw, dict) else getattr(raw, "candidate_id", None)),
                    source_candidate_id=(raw.get("source_candidate_id") if isinstance(raw, dict) else getattr(raw, "source_candidate_id", None)),
                    source_candidate_ids=list((raw.get("source_candidate_ids") if isinstance(raw, dict) else getattr(raw, "source_candidate_ids", [])) or []),
                    source_contour_ids=list((raw.get("source_contour_ids") if isinstance(raw, dict) else getattr(raw, "source_contour_ids", [])) or []),
                    candidate_origin=(raw.get("candidate_origin") if isinstance(raw, dict) else getattr(raw, "candidate_origin", None)),
                    contour_bridge_evidence=dict((raw.get("contour_bridge_evidence") if isinstance(raw, dict) else getattr(raw, "contour_bridge_evidence", {})) or {}),
                    contour_bridge_guard_reason_codes=list((raw.get("contour_bridge_guard_reason_codes") if isinstance(raw, dict) else getattr(raw, "contour_bridge_guard_reason_codes", [])) or []),
