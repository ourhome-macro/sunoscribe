# MP4->MIDI Benchmark Summary

- Created at: `2026-05-06T09:17:54.287639+00:00`
- Samples run: `0`
- Success: `0`
- Failed: `0`
- Mean note F1: `None`
- Mean pitch accuracy: `None`
- Readiness: `fail`

| Sample | Status | Note F1 | Pitch Acc | Onset MAE ms | Error |
| --- | --- | ---: | ---: | ---: | --- |

## Dataset Completeness

- MP4 files: `22`
- MIDI files: `23`
- Paired files: `19`
- Enabled samples: `19`
- MP4 only: `2`
- MIDI only: `4`

## MVP Readiness

- `python`: `ok` — Python 3.10 is recommended for backend runtime.
- `ffmpeg`: `ok` — ffmpeg is available.
- `pretty_midi`: `ok` — pretty_midi is importable.
- `mido`: `ok` — mido is importable.
- `librosa`: `ok` — librosa is importable.
- `soundfile`: `ok` — soundfile is importable.
- `rmvpe_pitch`: `fail` — RMVPE production runtime is not ready.
- `vocal_separator`: `fail` — Install `audio-separator` and matching onnxruntime for production vocal separation.
