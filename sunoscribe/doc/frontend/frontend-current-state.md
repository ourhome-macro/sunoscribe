...
- Project Detail：展示项目信息、当前分析状态、revision 列表、音频分析报告入口与 artifact 摘要。
...
- `AudioAnalysisPanel` 读取或生成当前 latest revision 的 `audio_analysis_report`，只展示结构化报告摘要，不直接读取后端 workspace 文件。
...
- 真实 API client 已包含音频分析报告的 `GET/POST /api/score-revisions/{revision_id}/audio-analysis` 方法，但文件上传进度、任务轮询和更多 artifact 下载仍未完成。
...
- 音频分析报告是可选解释层；缺歌词或缺某类证据时应展示 warning/空状态，不能把它渲染成 required stage failure。
